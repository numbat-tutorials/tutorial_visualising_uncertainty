...
2. be able to plot different types of uncertainty. 
3. assess whether one representation may be better than another.
4. apply these principles to spatial data visualisations.
...
- Error bands
- Gradients
- Multiple samples, such as bootstrap or simulation (not shown here)

## Exercise 3

forecast plots, and ask what is the version of uncertainty applied
...
```{r}
#| echo: false
# Variable names don't matter because these 
# need to be removed from any plots
nlev <- 8
x <- 1:nlev
...
#ggplot(struct_d, aes(x=x, y=y)) + geom_point()
#set.seed(118)
#noise1 <- tibble(x=x, y=rpois(nlev, 10))
#ggplot(noise1, aes(x=x, y=y)) + geom_point()
#struct_d_n <- struct_d + noise1
#ggplot(struct_d_n, aes(x=x, y=y)) + geom_point()
# Generate 11 nulls + data
set.seed(130)
noise_param <- 1
noise_scale <- 30
m <- 12 # multiple of 3
...
we are going to evaluate these plots 
